	...
	mailTransfer: async (idFrom, idTo) => {
		try {
			...
			const mailOptionsFrom = {
				...
				html: `<p> Hi ${accountFrom.fullname}! Your transaction to ${accountTo.fullname} was successful </p>
			...
			const mailOptionsTo = {
				...
				subject: "Transfer", //Subject mail
				html: `<p> Hi ${accountTo.fullname}! You have received a transaction from ${accountFrom.fullname} </p>
                    <a href="${FRONT_HOST}mywallet"> Click here </a>.`,
			};

			return (
				transporter.sendMail(mailOptionsFrom) &&
				transporter.sendMail(mailOptionsTo)
			);
		} catch (err) {
			console.error(err);
			throw new Error(err);
	...
	mailCardTransfer: async (from, to, card_num) => {
		try {
			...
			const mailOptionsTo = {
				...
				html: `<p> Hi ${
					accountTo.fullname
				}! Your account has received ${from} ${card_num
					.split("")
					.splice(0, card_num.split("").length - 4)
					.map((el) => el.replace(/[0-9]/, "*"))
					.join("")}${card_num.split("").splice([-4]).join("")}</p>
            <a href="${FRONT_HOST}mywallet"> Click here </a>.`,
			};
