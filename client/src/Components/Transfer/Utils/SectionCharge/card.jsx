...
const Card = () => {
  ...
  const handleAmount = (e) =>{setAmount(amount = e.target.value)}   
  ...
  const handleChange = (e) =>{
    ...
    updateinfo.account_id === '' && setUpdateInfo({
      ...updateinfo, account_id: store?.id
    })
  }
  ...
  const handleSubmit = async(e) => {
    ...
    try{
      ...
      const {data} = await axios.post(`transaction/card`, {...updateinfo, amount})
      await swal("Congratulations!", "Your upload has been successful!","success");
  ...
  return (
    <div className={cd.container} >
      ...
      {!modalCard?  
        <form onSubmit={ (e) => toggleModalCard(e)}  className={cd.containerTop}>
          ...
          <div className={cd.bottomListPrice}>
              { listPrice.map(el => <ViewPrice key={++keyNum} price={el} changePrice={handleAmount} />) }
          </div>
      ...
      :
        <div className={cd.modalCard} style={{display: "flex", flexDirection: "column", alignItems:"center"}}>
          ...
          <form className={cd.FormCard} onSubmit={(e) => handleSubmit(e)}>
            ...
            <button type='submit' className={cd.btnSubmitFormCard}>submit</button>
          </form>
